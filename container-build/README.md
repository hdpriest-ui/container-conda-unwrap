...
```bash
apptainer run --env PECAN_VERSION=1.11 --bind ~/envs:/mnt/envs --bind ~/packed:/mnt/packed ./pecan-build-rocky8.sif
```
...
```bash
aws s3 cp --endpoint-url https://s3.garage.ccmmf.ncsa.cloud ~/packed/pecan-all-1.11.tar.gz s3://carb/environments/pecan-all-1.11.tar.gz
```
...
```bash
bash setup-pecan-env.sh 1.11
```

An optional second argument sets the install path (default: `~/.conda/envs/pecan-all`):

```bash
bash setup-pecan-env.sh 1.11 /path/to/envs/pecan-all
```
